...
const HeaderManager = {
  ...
  /**
   * Asegura que una columna exista en una hoja específica.
   * Si no existe, la crea al final y refresca el caché.
   * @param {string} sheetAlias El alias de la hoja en SHEETS.
   * @param {string} columnName El nombre técnico de la columna.
   */
  ensureColumn(sheetAlias, columnName) {
    const mapping = this.getMapping(sheetAlias);
    if (mapping && mapping[columnName.toUpperCase()] !== undefined) return true;
    ...
    const lastCol = sheet.getLastColumn();
    sheet.getRange(1, lastCol + 1).setValue(columnName.toUpperCase());
    SpreadsheetApp.flush();
    
    this.clearCache(); // Forzamos recarga del mapeo
    console.log(`✅ [HeaderManager] Columna '${columnName}' creada en '${sheetName}'.`);
    return true;
  }
};
