...
function sincronizarImagenes(productoIdFiltro = null, logArray = null) {
  ...
  try {
    ...
    if (nuevasFilas.length > 0) {
      ...
      // --- NUEVO: AUTO-AUDITORÍA FORENSE PARA NUEVAS IMÁGENES ---
      // Se procesan secuencialmente mientras el tiempo de ejecución lo permita
      log(`🔎 Iniciando auto-auditoría forense para ${nuevasFilas.length} nuevas...`);
      for (const row of nuevasFilas) {
        // Verificar si nos acercamos al timeout (6 min es el límite de Apps Script)
        if ((Date.now() - startTime) > (MAX_EXECUTION_MS - 45000)) { // 45s de margen de seguridad
          log("⚠️ Límite de tiempo cerca. El resto de las nuevas se auditarán automáticamente en el próximo ciclo.");
          break;
        }
        
        const imgId = row[col['IMAGEN_ID']];
        const tipoArc = row[col['TIPO_ARCHIVO']];
        
        // Solo auditar imágenes, saltar videos
        if (imgId && tipoArc === 'imagen') {
          try {
            // Se usa forzar=false para no repetir análisis si por alguna razón ya existe
            escanearPrenda(imgId, false);
          } catch(e) {
            console.error(`[Auto-Forense] Fallo en ${imgId}: ${e.message}`);
          }
        }
      }
...
// =================================================================
// ===  CAPA 0 — MOTOR DE DIRECTIVAS ESTÉTICAS (SOT)             ===
// =================================================================
/**
 * Fuente de Verdad Única para las reglas de estilo de la IA.
 * Centraliza la lógica de Ecommerce, Ghost, Lifestyle, Folded y Hanger.
 */
function _getAiArtDirectionRules(estiloSolicitado, extraSpecs = {}, environment = 'Studio', prodRow = null) {
  ...
  switch (estilo) {
    case 'ghost':
      ...
      `;
      modelAdaptation = "- GENDER MANDATE: ABSOLUTELY NO HUMANS. Invisible Mannequin only.";
      break;

    case 'lifestyle':
      promptRules = `
        HIGH-END LIFESTYLE EDITORIAL:
        - High-quality fashion model wearing the garment in a natural environment.
    ...
    case 'hero':
    case 'collage':
      const isStillLifeStudio = environment === 'studio_minimalist';
      const isMarble = environment === 'luxury_marble';
      const isOak = environment === 'dark_oak';
      const isConcrete = environment === 'industrial_concrete';
      const isLinen = environment === 'soft_linen';

      const genero = (prodRow ? prodRow.GENERO : 'UNISEX').toUpperCase();

      // Props sugeridos por género (contexto de fondo opcional o sutil)
      let genderProps = "";
      ...
      // Definición de superficie según entorno seleccionado
      let surfaceInstruction = "a professional studio surface";
      if (isMarble) surfaceInstruction = "a polished luxury white marble surface with soft, realistic reflections";
      else if (isOak) surfaceInstruction = "a textured natural dark oak wood surface with a warm boutique feel";
      else if (isConcrete) surfaceInstruction = "a matte industrial concrete surface for a modern, high-contrast look";
      else if (isLinen) surfaceInstruction = "a soft, organic linen fabric background with natural folds";
      ...
      promptRules = `
        STILL LIFE PRODUCT PHOTOGRAPHY (PROFESSIONAL BOUTIQUE):
        ${compositionRules}
        - Background: ${isStillLifeStudio ? 'High-end photography studio (Neutral Soft Gray or Professional Off-White)' : 'Professional minimalist environment'}.
        - Surface: Set the garment(s) on ${surfaceInstruction}.
        - Lighting: Softbox multi-point lighting with professional highlights and soft falloff. Highlight technical fabric details.
        - Subject: ONLY the garment(s) in a professionally arranged composition.
        - REALISM MANDATE: Maintain realistic garment characteristics like seams, stitches, and natural fabric volume.
        - BOUTIQUE QUALITY: The garment MUST appear professionally steamed and neatly arranged. Remove any sloppy wrinkles or tags.
        - ABSOLUTELY NO MODELS, HUMAN BODIES, OR VISIBLE MANNEQUINS.
        - Style: High-fidelity advertising for premium catalogs.
      `;
      modelAdaptation = `- ABSOLUTELY NO HUMANS, MODELS, OR VISIBLE MANNEQUINS.
        - SUBJECT: Focus exclusively on the physical arrangement of the garment(s) as a Technical Still Life.`;
      break;
  ...
  let prefix = '[Final definitive photographic narrative description in technical English, focused purely on visual aspects].';
  ...
  return { promptRules, modelAdaptation, prefix };
}
...
function escanearPrenda(imagenId, forzar = false) {
  try {
    ...
    if (prodRow) {
      contextoProducto = `
      REFERENCE PRODUCT DATA (CONTEXT ONLY):
    ...
    const promptForense = `
      ...
      PIXEL SOVEREIGNTY MANDATE (CRITICAL):
      1. The photo is the ABSOLUTE AUTHORITY. 
      2. The "Colors from DB" list above is ONLY a reference of possible variants. DO NOT assume the garment in this specific photo matches the first color of that list.
      3. If the Database Metadata contradicts what you see in the pixels (e.g., DB says Red but the photo is Grey), you MUST report what you SEE (Grey).
      4. Precisely identify textures, patterns (animal print, camo, etc.), and materials.

      Instructions:
      Report the following attributes in Technical English:

      GARMENT_TYPE: [e.g., T-shirt, Pants, Jacket, Bomber, Hoodie]
      DETECTED_POSITION: [FRONT / BACK / SIDE / FLAT_LAY / GHOST_MANNEQUIN / STACKED / UNDETERMINED]
      SUPPORT_CONTEXT: [STUDIO_PHOTO / ON_HANGER / ON_SURFACE / ON_MANNEQUIN / ON_RACK]
      PRIMARY_COLOR:
        - Technical Name: [e.g., Navy Blue, Charcoal Gray, Off-White]
        - HEX Code: [e.g., #1A2B5C]
        - Type: [SOLID / PRINTED / SUBLIMATED / STRIPED / HEATHERED / ANIMAL_PRINT]
        - Pattern Details: [Describe texture or print if applicable]
      ESTIMATED_MATERIAL: [e.g., Cotton Jersey, Dri-FIT, Eco-leather, Denim, Knit]
      LOGOS_OR_BRANDING:
        - Visible: [YES / NO]
        - If YES: Exact description, position, and approximate size.
      CONSTRUCTION_DETAILS:
        - Stitching: [e.g., Flatlock, Overlock, Double needle]
        - Closures: [e.g., YKK Front zipper, Snap buttons, No closure]
        - Pockets: [e.g., 2 side zip pockets, No pockets]
        - Elastics: [e.g., Ribbed cuffs, Elastic waistband with drawstring]
      VISIBLE_CARE_LABELS: [YES / NO]
      VISUAL_STATE: [CLEAN and PRESENTABLE / Visible tags needing retouch / Mannequin visible]
    `;
...
/**
 * Garantiza que la columna ANALISIS_FORENSE exista en la hoja de imágenes.
 * @param {GoogleAppsScript.Spreadsheet.Sheet} sheet (Opcional, se usa el alias por defecto)
 */
function garantizarColumnaANALISIS() {
  HeaderManager.ensureColumn("PRODUCT_IMAGES", "ANALISIS_FORENSE");
}

/* ================================================================= */
/* ===       PHASE 4 — UNIFIED PROMPT ENGINE (CORE v2.0)         === */
/* ================================================================= */

/**
 * Motor centralizado de generación de prompts técnicos.
 * Resuelve la contaminación de contexto y estandariza el idioma en INGLÉS.
 */
function _promptEngineCore(imageIds, estiloSolicitado, modo = 'image', extraSpecs = {}, pin = null) {
  const logPrefix = `🧠 [Prompt-Engine]`;
  const ids = Array.isArray(imageIds) ? imageIds : [imageIds];
  console.log(`${logPrefix} Iniciando análisis unificado para ${ids.length} imágenes.`);
  ...
  try {
    ...
    // 1. OBTENER Y VALIDAR IMÁGENES
    const selectedRows = dataImg.filter(r => ids.includes(String(r.IMAGEN_ID).trim()));
    if (selectedRows.length === 0) throw new Error("No se encontraron registros de las imágenes seleccionadas.");

    const skuBase = selectedRows[0].PRODUCTO_ID;
    
    // Alerta de inconsistencia de SKU
    const skusDiferentes = [...new Set(selectedRows.map(r => r.PRODUCTO_ID))];
    if (skusDiferentes.length > 1) {
      console.warn(`${logPrefix} ⚠️ Mezcla de SKUs detectada: ${skusDiferentes.join(", ")}. Usando contexto de: ${skuBase}`);
    }

    // 2. OBTENER METADATOS TÉCNICOS SOT (Source of Truth)
    const sheetProd = ss.getSheetByName(SHEETS.PRODUCTS);
    const dataProd = convertirRangoAObjetos_IMAGENES(sheetProd);
    const prodRow = dataProd.find(p => String(p.CODIGO_ID || p.SKU).trim() === String(skuBase).trim());

    let technicalContext = "Product: Apparel Item.";
    if (prodRow) {
      let colorsDb = prodRow.COLORES || "";
      if (colorsDb.toLowerCase().includes("surtido")) colorsDb = "Various (Priority: Rely ONLY on the visual pixels detected).";
      ...
      technicalContext = `
        ...
        - Reference Colors: ${colorsDb}
        - Features: ${prodRow.DESCRIPCION || "Modern style"}
      `;
    }

    // 3. AGREGAR ANÁLISIS FORENSE (Multimodal Insight)
    let aggregatedForensics = "";
    selectedRows.forEach((r, idx) => {
      if (r.ANALISIS_FORENSE) {
        // Limpiamos etiquetas de idioma previas si existen
        const cleanForensic = r.ANALISIS_FORENSE.replace(/GARMENT_TYPE:|PRIMARY_COLOR:/gi, (m) => `\n${m}`);
        aggregatedForensics += `\n[Reference Image ${idx + 1} Insights]:\n${cleanForensic}\n`;
      }
    });

    const forensicMandate = aggregatedForensics 
      ? `\nVISUAL FORENSIC AUDIT (TRUTH OVERRIDE):\n${aggregatedForensics}\n- MANDATE: If the visual pixels in these reports (Reference Images) contradict the Database Metadata (e.g., color mismatch), you MUST prioritize the visual evidence detected in the photos.`
      : "\n- MANDATE: Extract color, texture, and silhouette strictly from the provided image pixels.";

    // 4. CONFIGURAR DIRECTIVA DE ARTE
    const estiloSolicitadoL = (estiloSolicitado || 'ecommerce').toLowerCase();
    const environment = extraSpecs.environment || (prodRow ? prodRow.CATEGORIA_PADRE : 'Studio');
    const direction = _getAiArtDirectionRules(estiloSolicitadoL, extraSpecs, environment, prodRow);
    const forcedAngle = extraSpecs.angle || "";

    const systemPrompt = `
      ROLE: Expert Fashion Art Director & Prompt Engineer (Industrial Grade).
      GOAL: Create a hyper-realistic narrative description for image generation.
      
      VISUAL AUTHORITY:
      1. Technical Metadata: ${technicalContext}
      2. Forensic Audit: ${forensicMandate}
      
      CORE FIDELITY RULES:
      - LOGO SANCTITY: Do NOT add logos unless they are clearly visible or specified in forensic data.
      - TEXTURE FIDELITY: Describe weave, fabric tension, and material highlights.
      - TAG REMOVAL: Strip all retail tags, hangers, and cardboard stickers.
      - NO HYBRIDS: If multiple images show different items by mistake, ignore the outliers and focus on the SKU: ${skuBase}.
      
      ARTISTIC DIRECTION:
      ${direction.promptRules}
      ${direction.modelAdaptation}
      ${forcedAngle ? `\n- TARGET VIEWPOINT: Render the final image from the **${forcedAngle}** angle, regardless of the reference orientation.` : ""}

      OUTPUT REQUIREMENT (MANDATORY):
      - LANGUAGE: **ALL FIELDS MUST BE IN ENGLISH**. No Spanish allowed in the final output.
      
      OUTPUT STRUCTURE (JSON):
      {
        "TECHNICAL_REASONING": "Step-by-step logic on texture and color extraction.",
        "FORENSIC_ALIGNMENT": "Report on alignment between photos and DB metadata.",
        "MASTER_PROMPT": "${direction.prefix} [Detailed architectural description of the garment, lighting, and environment].",
        "GENERATION_SPECS": "Model used, aspect ratio, and technical seeds."
      }
    `;

    // 5. PREPARAR IMÁGENES (Multimodal)
    const apiKey = GLOBAL_CONFIG.GEMINI.API_KEY;
    const contentsParts = [];
    
    selectedRows.slice(0, 5).forEach((r, i) => {
      try {
        const fileData = prepararBlobOptimizado(r.ARCHIVO_ID, `core_ref_${i}`, 'alta', apiKey, true);
        contentsParts.push(fileData);
      } catch (err) { console.warn(`${logPrefix} Fail ID ${r.IMAGEN_ID}: ${err.message}`); }
    });

    // 6. EJECUCIÓN CON FALLBACK
    const modelos = ["gemma-3-27b-it", "gemini-2.0-flash", "gemini-2.5-flash"];
    let errores = [];

    for (const mod of modelos) {
      try {
        const url = `https://generativelanguage.googleapis.com/v1beta/models/${mod}:generateContent?key=${apiKey}`;
        const payload = mod.startsWith('gemini')
          ? { "systemInstruction": { "parts": [{ "text": systemPrompt }] }, "contents": [{ "parts": [...contentsParts] }] }
          : { "contents": [{ "parts": [{ "text": systemPrompt }, ...contentsParts] }] };
        ...
        if (resp.getResponseCode() === 200) {
          ...
          if (text) {
            ...
            console.log(`${logPrefix} ✅ Éxito con ${mod}`);
            return { success: true, response: cleanText, model: mod, sku: skuBase };
          }
        } else {
          errores.push(`${mod}: ${resp.getContentText()}`);
        }
      } catch (e) { errores.push(`${mod} EX: ${e.message}`); }
    }
    throw new Error(`Fallo Total Engine: ${errores.join(" | ")}`);
...
function generarSuperPrompt(imagenId, estiloSolicitado, modo = 'image', extraSpecs = {}, pin = null) {
  if (modo === 'video') return generarVideoPrompt([imagenId], estiloSolicitado, { extraSpecs: extraSpecs });

  const coreRes = _promptEngineCore([imagenId], estiloSolicitado, modo, extraSpecs, pin);
  if (!coreRes.success) return JSON.stringify(coreRes);

  const promptGenerado = coreRes.response;
  actualizarCeldaPorHeader(imagenId, 'PROMPT', promptGenerado);

  const resObj = { success: true, text: promptGenerado, model: coreRes.model };

  // INTEGRACIÓN RENDER (Si aplica)
  if (modo === 'image' && pin) {
    try {
      ...
      if (resImg.success) {
        ...
        resObj.text += `\n\n✅ Image generated with ${resImg.modelUsed}.`;
      }
...
// 🔍¥ NUEVA FUNCIÓN: GENERACIÓN MULTIMODAL (VARIAS IMÁGENES) - REFACTORIZADA v2.0
function generarSuperPromptMasivo(imageIds, estiloSolicitado, modo = 'image', extraSpecs = {}, pin = null) {
  ...
  // Puente hacia Video (Si aplica)
  if (modo === 'video') return generarVideoPrompt(imageIds, estiloSolicitado, { extraSpecs: extraSpecs });

  // LLAMADA AL MOTOR UNIFICADO (Resuelve contaminación y estandariza Inglés)
  const coreRes = _promptEngineCore(imageIds, estiloSolicitado, modo, extraSpecs, pin);
  if (!coreRes.success) return JSON.stringify(coreRes);

  const promptGenerado = coreRes.response;

  // Actualizar todos los registros involucrados con el nuevo prompt maestro
  imageIds.forEach(id => {
  ...
  if (modo === 'image' && pin) {
    try {
      ...
      if (resImg.success) {
        resObj.imageSuccess = true;
        resObj.imagenId = resImg.imagenId;
        resObj.text += `\n\n✅ Master Image generated with ${resImg.modelUsed}.`;
      }
    } catch (e) { resObj.text += `\n\n❌ Render Error: ${e.message}`; }
  }
